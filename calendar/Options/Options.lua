...
function UnpackList(input)
	...
	for _, StyleName in ipairs(StyleList) do
		local ini = ReadIni(SKIN:ReplaceVariables(string.format('%sCalendar\\Styles\\%s\\meta.txt', Resources, StyleName)))
		local temp
...
function Save()
	MessageOutput()
	local SettingsFile = SKIN:ReplaceVariables('#@#Calendar\\Settings.inc')
